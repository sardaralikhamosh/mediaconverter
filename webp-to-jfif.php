...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_FILES['webpFile'])) {
    ...
    if ($file['type'] == 'image/webp') {
        ...
        if (move_uploaded_file($file['tmp_name'], $webpPath)) {
            ...
            $jfifPath = 'converted/' . pathinfo($webpPath, PATHINFO_FILENAME) . '.jfif';

            // Convert WEBP to JFIF using PHP's GD library
            $webpImage = imagecreatefromwebp($webpPath);
            if ($webpImage) {
                // Create a new JFIF image from the WEBP image
                if (imagejpeg($webpImage, $jfifPath, 100)) {
                ...
                $uploadError = 'Error converting WEBP to JFIF.';
            }
        } else {
...
<body>
  ...
  <div class="container d-flex align-items-center justify-content-center" style="height: 100vh; margin-top: 0;">
        <div class="content" style="width: 100%; max-width: 600px;">
            <h2 class="text-center">Convert WEBP to JFIF</h2>
            <p class="text-center">Upload a WEBP file to convert it to JFIF format.</p>
            ...
            <?php if ($downloadLink): ?>
                <div class="alert alert-success" style="margin-top: 20px;">
                    ...
                    <a href="webp-to-jfif.php" class="btn btn-secondary btn-block" style="margin-top: 10px;">Go Back</a>
                </div>
            <?php endif; ?>
        </div>
    </div>
