...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_FILES['jfifFile'])) {
    ...
    if ($fileType === 'image/jpeg') {
        ...
        if (move_uploaded_file($file['tmp_name'], $jfifPath)) {
            ...
            $jpgPath = 'converted/' . pathinfo($jfifPath, PATHINFO_FILENAME) . '.jpg';

            // Convert JFIF to JPG using PHP's GD library
            $jfifImage = imagecreatefromjpeg($jfifPath);
            if ($jfifImage) {
                if (imagejpeg($jfifImage, $jpgPath)) {
                ...
                $uploadError = 'Error converting JFIF to JPG.';
            }
        } else {
...
<body>
  ...
  <div class="container d-flex align-items-center justify-content-center" style="height: 100vh; margin-top: 0;">
        <div class="content" style="width: 100%; max-width: 600px;">
            <h2 class="text-center">Convert JFIF to JPG</h2>
            <p class="text-center">Upload a JFIF file to convert it to JPG format.</p>
            
            <form method="post" enctype="multipart/form-data">
                <div class="form-group">
                    <label for="jfifFile">Choose JFIF file:</label>
                    <input type="file" name="jfifFile" class="form-control" id="jfifFile" accept=".jfif" required>
                </div>
            ...
            <?php if ($downloadLink): ?>
                <div class="alert alert-success" style="margin-top: 20px;">
                    ...
                    <a href="jfif-to-jpg.php" class="btn btn-secondary btn-block" style="margin-top: 10px;">Go Back</a>
                </div>
            <?php endif; ?>
        </div>
    </div>
