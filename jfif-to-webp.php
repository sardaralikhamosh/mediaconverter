...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_FILES['jfifFile'])) {
    ...
    if ($file['type'] == 'image/jpeg' || pathinfo($file['name'], PATHINFO_EXTENSION) === 'jfif') {
        ...
        if (move_uploaded_file($file['tmp_name'], $jfifPath)) {
            ...
            $webpPath = 'converted/' . pathinfo($jfifPath, PATHINFO_FILENAME) . '.webp';

            // Create a new image resource from the JFIF file
            try {
                $imagick = new Imagick();
                $imagick->readImage($jfifPath);
                ...
                $uploadError = 'Error converting JFIF to WEBP: ' . $e->getMessage();
            }
        } else {
  ...
  <!-- Body starts -->
   <div class="container d-flex align-items-center justify-content-center" style="height: 100vh; margin-top: 0;">
        <div class="content" style="width: 100%; max-width: 600px;">
            <h2 class="text-center">Convert JFIF to WEBP</h2>
            <p class="text-center">Upload a JFIF file to convert it to WEBP format.</p>
            ...
            <?php if ($downloadLink): ?>
                <div class="alert alert-success" style="margin-top: 20px;">
                    ...
                    <a href="jfif-to-webp.php" class="btn btn-secondary btn-block" style="margin-top: 10px;">Go Back</a>
                </div>
            <?php endif; ?>
        </div>
    </div>
