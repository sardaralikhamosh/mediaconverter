...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_FILES['avifFile'])) {
    $file = $_FILES['avifFile'];
    ...
    if ($file['type'] == 'image/avif') {
        ...
        if (move_uploaded_file($file['tmp_name'], $avifPath)) {
            ...
            $pdfPath = 'converted/' . pathinfo($avifPath, PATHINFO_FILENAME) . '.pdf';

            // Convert AVIF to PDF using Imagick
            try {
                $imagick = new Imagick();
                $imagick->readImage($avifPath); // Read the AVIF image
                $imagick->setImageFormat('pdf'); // Set format to PDF
                $imagick->writeImage($pdfPath); // Write to PDF file
                $imagick->clear();
                $imagick->destroy();
...
<head>
    ...
    <title>Convert AVIF to PDF</title>
...
<body>
 ...
 <div class="container d-flex align-items-center justify-content-center" style="height: 100vh; margin-top: 0;">
        <div class="content" style="width: 100%; max-width: 600px;">
            <h2 class="text-center">Convert AVIF to PDF</h2>
            <p class="text-center">Upload an AVIF file to convert it to PDF format.</p>
            
            <form method="post" enctype="multipart/form-data">
                <div class="form-group">
                    <label for="avifFile">Choose AVIF file:</label>
                    <input type="file" name="avifFile" class="form-control" id="avifFile" accept=".avif" required>
                </div>
            ...
            <?php if ($downloadLink): ?>
                <div class="alert alert-success" style="margin-top: 20px;">
                    ...
                    <a href="avif-to-pdf.php" class="btn btn-secondary btn-block" style="margin-top: 10px;">Go Back</a>
                </div>
            <?php endif; ?>
        </div>
    </div>
