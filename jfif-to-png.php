...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_FILES['jfifFile'])) {
    $file = $_FILES['jfifFile'];

    // Check if the uploaded file is a JFIF image
    if ($file['type'] == 'image/jpeg' || pathinfo($file['name'], PATHINFO_EXTENSION) === 'jfif') {
        $uploadDir = 'uploads/';
        ...
        if (move_uploaded_file($file['tmp_name'], $jfifPath)) {
            ...
            $pngPath = 'converted/' . pathinfo($jfifPath, PATHINFO_FILENAME) . '.png';

            // Convert JFIF to PNG using PHP's GD library
            $jfifImage = imagecreatefromjpeg($jfifPath);
            if ($jfifImage) {
                if (imagepng($jfifImage, $pngPath)) {
                    imagedestroy($jfifImage); // Free up memory
                    $downloadLink = $pngPath; // Set the path for the download button
                } else {
                    $uploadError = 'Error saving PNG file.';
                }
            } else {
                $uploadError = 'Error converting JFIF to PNG.';
            }
        } else {
            $uploadError = 'Error uploading file.';
        }
    } else {
        $uploadError = 'Please upload a JFIF file.';
    }
}
?>
  ...
  <!-- Body starts -->
   <div class="container d-flex align-items-center justify-content-center" style="height: 100vh; margin-top: 0;">
        <div class="content" style="width: 100%; max-width: 600px;">
            <h2 class="text-center">Convert JFIF to PNG</h2>
            <p class="text-center">Upload a JFIF file to convert it to PNG format.</p>
            
            <form method="post" enctype="multipart/form-data">
                <div class="form-group">
                    <label for="jfifFile">Choose JFIF file:</label>
                    <input type="file" name="jfifFile" class="form-control" id="jfifFile" accept=".jfif, .jpeg" required>
                </div>
            ...
            <?php if ($downloadLink): ?>
                <div class="alert alert-success" style="margin-top: 20px;">
                    ...
                    <a href="jfif-to-png.php" class="btn btn-secondary btn-block" style="margin-top: 10px;">Go Back</a>
                </div>
            <?php endif; ?>
        </div>
    </div>
