<?php
session_start(); // Start session to manage download link state
...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_FILES['jpgFile'])) {
    ...
    if ($file['type'] == 'image/jpeg' || $file['type'] == 'image/jpg') {
        ...
        if (move_uploaded_file($file['tmp_name'], $jpgPath)) {
            // Set the PDF path
            $pdfPath = 'converted/' . pathinfo($jpgPath, PATHINFO_FILENAME) . '.pdf';

            // Convert JPG to PDF using Imagick
            try {
                ...
                $uploadError = 'Error converting JPG to PDF: ' . $e->getMessage();
            }
        } else {
...
<body>
 ...
 <div class="container d-flex align-items-center justify-content-center" style="height: 100vh; margin-top: 0;">
        <div class="content" style="width: 100%; max-width: 600px;">
            ...
            <p class="text-center">Upload a JPG file to convert it to PDF format.</p>
            ...
            <?php if ($downloadLink): ?>
                <div class="alert alert-success" style="margin-top: 20px;">
                    ...
                    <a href="jpg-to-pdf.php" class="btn btn-secondary btn-block" style="margin-top: 10px;">Go Back</a>
                </div>
            <?php endif; ?>
        </div>
    </div>
