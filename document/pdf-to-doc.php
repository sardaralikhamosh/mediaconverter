...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_FILES['pdfFile'])) {
    ...
    if ($fileType === 'application/pdf') { // Corrected MIME type
        ...
        if (move_uploaded_file($file['tmp_name'], $pdfPath)) {
            // Set the output path for the converted DOCX file
            $docxPath = $convertedDir . pathinfo($pdfPath, PATHINFO_FILENAME) . '.docx';

            // Convert PDF to DOCX using a library like PhpOffice\PhpWord
            try {
                require_once __DIR__ . '/../vendor/autoload.php'; // Load the required libraries

                // Load the PDF file
                $pdf = new \Smalot\PdfParser\Parser();
                $parsedPdf = $pdf->parseFile($pdfPath);
                ...
                $downloadLink = $docxPath; // Set the path for the download button
            } catch (Exception $e) {
                $uploadError = 'Error converting PDF to DOCX: ' . $e->getMessage();
            }
        } else {
...
<body>
    ...
    <div class="container d-flex align-items-center justify-content-center" style="height: 100vh; margin-top: 0;">
        <div class="content" style="width: 100%; max-width: 600px;">
            <h2 class="text-center">Convert PDF to DOCX</h2>
            <p class="text-center">Upload a PDF file to convert it to DOCX format.</p>
            ...
            <?php if ($downloadLink): ?>
                <div class="alert alert-success" style="margin-top: 20px;">
                    <p>Conversion successful! Download your DOCX file:</p>
                    <a href="<?= htmlspecialchars($downloadLink) ?>" class="btn btn-success btn-block" download>Download DOCX</a>
